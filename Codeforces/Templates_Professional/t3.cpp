...
void solve(int tc)
{
    def(int, x);
    def(ll, m);
    // divisible by min(x, y)
    ll ans = 0;
    FOR(y, 1, x)
    if (y <= m) if ((x ^ y) % y == 0)++ ans;
    ++m;
    vl dp_less(x);
    auto adv = [&](ll r)
    {
        r %= x;
        auto ndp = dp_less;
        F0R(i, x) { ndp.at((i + r) % x) += dp_less.at(i); }
        swap(ndp, dp_less);
    };
    auto getbit = [&](ll m, int k)
    { return m & (1LL << k); };
    ll sofar = 0;
    R0F(k, 60)
    {
        adv(1LL << k);
        if (getbit(m, k))
            ++dp_less.at((sofar + getbit(x, k)) % x);
        sofar += getbit(m, k) ^ getbit(x, k);
    }
    ans += dp_less.at(0);
    // F0R(y, m + 1) if ((x ^ y) % x == 0)++ ans;
    --ans;
    ps(ans);
}
