...
long long maxSubarrays(int n, vector<vector<int>> &conflictingPairs)
{
    ...
    vector<long long> extra(n + 1, 0); // extra[i] = no. of extra subarrays by removing conflicting point i

    for (int end = 1; end <= n; end++)
    {
        // visiting each point and treating them as ending point of subarray
