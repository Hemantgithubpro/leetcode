...
bool isparent(int u, int v, vector<int> &inTime, vector<int> &outTime)
{
...
int minimumScore(vector<int> &nums, vector<vector<int>> &edges)
{
    ...
    for (int u = 1; u < n; u++)
    {
        for (int v = u + 1; v < n; v++)
        {
            ...
            if (isparent(u, v, inTime, outTime))
            {
                xor1 = subtreeXor[v];
                xor2 = subtreeXor[u] ^ subtreeXor[v];
                xor3 = subtreeXor[0] ^ xor1 ^ xor2;
            }
            else if (isparent(v, u, inTime, outTime))
            {
                xor1 = subtreeXor[u];
            ...
            { // simplest case no parents, all independent
                xor1 = subtreeXor[u];
                xor2 = subtreeXor[v];
                xor3 = subtreeXor[0] ^ xor1 ^ xor2;
            }
