#include <bits/stdc++.h>
using namespace std;
// not the most efficient: O(n.L^2)
class Solution
{
public:
    vector<string> removeSubfolders(vector<string> &folder)
    {
        ...
        for (string &f : folder)
        {
            ...
            while (!prefix.empty())
            {
                int pos = prefix.find_last_of('/'); // Finds the last position in the string (up to __pos) where any character from __str occurs. Returns: The index of the last occurrence, or npos if none found.
                if (pos == string::npos)
                    break;
